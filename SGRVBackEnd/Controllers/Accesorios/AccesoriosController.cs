...
public sealed class AccesoriosController : BaseApiController
{
    ...
    [HttpPatch("{id:int}/restaurar"),Authorize(Roles=Managers)] public async Task<ActionResult<ApiResponse<object>>> Restore(int id,CancellationToken ct)=>await _service.SetCatalogActiveAsync(id,GetEmpresaId(),true,ct)?Ok(OkResponse<object>(new{id,Activo=true},"Accesorio restaurado correctamente.")):NotFound(Fail<object>("No se encontró un accesorio privado de la empresa."));
